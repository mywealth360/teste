/*
  # Financial Goals System with Bill Integration

  1. New Tables
    - `financial_goals` - Store user financial goals
    - `goal_contributions` - Track contributions to goals
    - `goal_recommendations` - Store AI-generated savings recommendations

  2. New Functions
    - `handle_goal_status_change` - Handle goal completion/abandonment
    - `mark_bill_as_paid_with_goal` - Process bill payment with goal contribution
    - `create_goal_contribution_bill` - Create recurring bills for goals

  3. Modifications
    - Add financial_goal_id and is_goal_contribution to bills table
...
-- Drop the policy first if it exists to avoid the error
...
-- Drop the policy first if it exists to avoid the error
DROP POLICY IF EXISTS "Users can manage their own goal contributions" ON goal_contributions;
...
-- Drop the policy first if it exists to avoid the error
DROP POLICY IF EXISTS "Users can manage their own goal recommendations" ON goal_recommendations;
