/*
  # Bill Alert System with Automatic Scheduling

  1. New Functions
    - `schedule_bill_alert` - Automatically schedules alerts 1 day before bill due dates
    - `update_bill_alert_status` - Updates alert status when bills are paid
    - `mark_bill_as_paid_with_alert` - Marks bills as paid and updates related alerts
    - `schedule_bill_alert_for_id` - Manually schedules an alert for a specific bill
    - `update_all_bill_alerts` - Updates all bill alerts (for admin or manual refresh)

  2. Triggers
    - Trigger to automatically schedule alerts when bills are created or updated
    - Trigger to update alert status when bills are marked as paid
...
-- Create function to mark multiple bills as paid
...
BEGIN
  ...
  LOOP
    IF mark_bill_as_paid_with_alert(bill_id, payment_date_val) THEN
