/*
  # Create Invites Table for User Access Sharing

  1. New Tables
    - `invites` - Stores invitations for shared access to accounts
      - Includes owner_id, email, role, token, and expiration
      - Supports viewer, editor, and admin roles

  2. Functions
    - `generate_invite_token()` - Creates unique tokens for invites
    - `create_invite()` - Creates new invites with validation

  3. Security
    - Enable RLS on invites table
    - Add policy for users to manage their own invites
    - Validate user has family plan before allowing invites
*/
